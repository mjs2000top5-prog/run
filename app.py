...
def get_suwon_hourly_weather():
    ...
    params = {
        ...
        "pageNo": "1",
        "numOfRows": "250", # 시간대별 조회를 위해 데이터 가져오는 양을 늘렸습니다.
        "dataType": "JSON",
        "base_date": base_date,
        "base_time": base_time,
        "nx": str(NX),
    ...
    try:
        ...
        # 모바일에 알맞게 데이터프레임용 리스트로 가공
        parsed_records = []
        for (f_date, f_time), categories in sorted(hourly_dict.items()):
            # 모바일 화면 길이를 고려하여 향후 12시간만 표기
            if len(parsed_records) >= 12:
                break
                
            # 날짜 가독성 처리 (오늘/내일)
            f_date_obj = datetime.datetime.strptime(f_date, "%Y%m%d").date()
...
with main_tabs[0]:
    ...
    # 시간대별 날씨 호출 및 데이터프레임 출력
    weather_df = get_suwon_hourly_weather()
    
    # 모바일 꽉 차는 스크롤 표로 레이아웃 구성
    st.dataframe(
        weather_df,
    ...
    # 실시간 강수 여부 체크하여 알림 가이드 제공
    if not weather_df.empty and "강수량" in weather_df.columns:
        next_rain = weather_df[weather_df["강수량"] != "0mm"].first_valid_index()
        if next_rain is not None:
            rain_time = weather_df.loc[next_rain, '시간']
            st.warning(f"⚠️ {weather_df.loc[next_rain, '일자']} {rain_time}에 비 예보가 있습니다! 실내 운동을 추천해요.")
        else:
            st.info("💡 향후 몇 시간 동안 비 소식이 없습니다. 야외 운동하기 좋습니다!")
