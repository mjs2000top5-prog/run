...
# --- 3. 기상청 단기예보 API 연동 (현재 조회시간 기준 이후 12시간 필터링) ---
@st.cache_data(ttl=900) # 15분간 날씨 결과 캐싱
def get_suwon_hourly_weather():
    ...
    params = {
        ...
        "pageNo": "1",
        "numOfRows": "300", # 12시간 분량을 안정적으로 확보하기 위해 넉넉히 요청
        "dataType": "JSON",
        "base_date": base_date,
        "base_time": base_time,
        "nx": str(NX),
    ...
    try:
        ...
        for (f_date, f_time), categories in sorted(hourly_dict.items()):
            ...
            # 📌 [핵심] 예보 시간이 현재 정각보다 과거라면 표기에서 제외
        ...
        # API 통신에 실패하거나 점검 중일 때 보여줄 빈 표
        return pd.DataFrame([
            {"일자": "오늘", "시간": "현재", "날씨": "🔄", "온도": "날씨 연동 확인 중", "강수량": "0mm"}
        ])
...
with main_tabs[0]:
    ...
    # 현재 시간 기준 이후 12시간 데이터프레임 호출 및 출력
    ...
    if not weather_df.empty and "강수량" in weather_df.columns:
        ...
        if not rain_rows.empty:
            ...
            st.warning(f"⚠️ {weather_df.loc[first_rain_idx, '일자']} {weather_df.loc[first_rain_idx, '시간']}에 비 예보({weather_df.loc[first_rain_idx, '강수량']})가 있습니다! 우산을 챙기세요.")
        else:
